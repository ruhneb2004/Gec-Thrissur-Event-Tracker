...
// import { useMarkerStore } from "@/app/store/markerStore";

export function DateTimePicker24h({ type }: { type: string }) {
  const { startDate, endDate, setStartDate, setEndDate } = useDateStore();
  // const { selectedMarker } = useMarkerStore();
  let date;
  if (type === "start") {
    date = startDate;
  } else {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="z-[1002] w-auto p-0 max-h-[80vh] overflow-y-auto">
